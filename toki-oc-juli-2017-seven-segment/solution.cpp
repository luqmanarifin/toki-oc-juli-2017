...
void setup(){
	...
	if(lenArr > n) maks_from_awal = false;
	else{
...
int rek(int pos, bool still_max){
	if(pos == n) return 1;
	if(dp[pos][still_max] == -1){
		int &ret = dp[pos][still_max] = 0;
		if(still_max){
			int max_num_pos = max_num[pos];
			if(val[pos] != -1){
				if(val[pos] > max_num_pos) ret = 0;
				else {
					ret = rek(pos + 1, val[pos] == max_num_pos);
				}
			}
			else{
				for(int i = 0;i <= max_num_pos; ++i){
					if(ar[pos].compatible(SEG[i])){
						ret += rek(pos + 1, i == max_num_pos);
						if(ret >= MOD) ret -= MOD;
					}
				}	
			}
